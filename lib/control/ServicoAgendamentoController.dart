...
import 'package:app/model/servicoagendamento.dart';

class Servicoagendamentocontroller {
  ...
  Future<int> addUser(ServicoAgendamento servicoagendamento) async {
    ...
    return await db.insert('ServicoAgendamento', servicoagendamento.toMap());
  }
  ...
  Future<int> editarUsuario(ServicoAgendamento servicoagendamento) async {
    // criar variavel de banco de para acessar os metodos
    final db = await bancodedados.database;
    // atualizar os dados do usuario no banco de dados
    return await db.update('ServicoAgendamento', servicoagendamento.toMap(),
        where: 'id=?', whereArgs: [servicoagendamento.idServicoAgen]);
  }

  Future<List<ServicoAgendamento>> getServicoAgendamento() async {
    ...
    final List<Map<String, dynamic>> listadeusuario =
        await db.query('ServicoAgendamento');
    ...
    return List.generate(listadeusuario.length, (int i) {
      return ServicoAgendamento.fromMap(listadeusuario[i]);
    });
  }
}
