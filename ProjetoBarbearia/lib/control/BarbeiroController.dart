...
class Barbeirocontroller {
  ...
  Future<int> editarUsuario(Barbeiro barbeiro) async {
    // criar variavel de banco de para acessar os metodos
    final db = await bancodedados.database;
    // atualizar os dados do usuario no banco de dados
    return await db.update('Barbeiro', barbeiro.toMap(),
        where: 'id=?', whereArgs: [barbeiro.idUsuario]);
  }
  ...
  Future<List<Barbeiro>> getBarbeiro() async {
    ...
    return List.generate(listadeusuario.length, (int i) {
      return Barbeiro.fromMap(listadeusuario[i]);
    });
  }
}
