...
class Clientecontroller {
  ...
  Future<int> addUser(Cliente cliente) async {
  ...
  Future<int> editarUsuario(Cliente cliente) async {
    // criar variavel de banco de para acessar os metodos
    final db = await bancodedados.database;
    // atualizar os dados do usuario no banco de dados
    return await db.update('Cliente', cliente.toMap(),
        where: 'id=?', whereArgs: [cliente.idCliente]);
  }
  ...
  Future<List<Cliente>> getCliente() async {
    ...
    final List<Map<String, dynamic>> listadeusuario = await db.query('Cliente');
    ...
    return List.generate(listadeusuario.length, (int i) {
      return Cliente.fromMap(listadeusuario[i]);
    });
  }
}
