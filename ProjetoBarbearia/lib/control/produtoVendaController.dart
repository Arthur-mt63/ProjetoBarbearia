...
Future<int> addprodutoVenda(ProdutoVenda produtoVenda)async{
...
Future<int> deletarProdutoVenda(int id) async{
//  Cria a variavel de banco de dados para acessar os metodos

final db = await bancodedados.database;
// Excluir o usuario dentro do banco de dados

return await db.delete('ProdutoVendav', where: 'id=?' ,whereArgs: [id] );
...
Future<int> editar(ProdutoVenda produtoVenda) async{


// criar variavel de banco de dados para acessar os metodos 
final db = await bancodedados.database;
// atualizar os dados do usuario no bancode dados
return await db.update('ProdutoVenda', produtoVenda.toMap(), where:  'id= ?', whereArgs: [produtoVenda.idProdutoVenda]);//atualiza dentro da tabela usuario onde oos dados do meus usuario onde for igual a minha tabela usuario
...
final List<Map<String , dynamic>> listadeProdutoVenda= await db.query('ProdutoVenda');
...
return List.generate(listadeProdutoVenda.length, (i){
  return ProdutoVenda.fromMap(listadeProdutoVenda[i]);
});
