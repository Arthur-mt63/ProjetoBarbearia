...
// start no banco de dados , criando um objeto dele;
  ...
  Future<int> addUser(Venda venda) async {
    ...
    return await db.insert('Venda', venda.toMap());
  }
  ...
  Future<int> editarUsuario(Venda venda) async {
    // criar variavel de banco de para acessar os metodos
    final db = await bancodedados.database;
    // atualizar os dados do usuario no banco de dados
    return await db.update('Venda', venda.toMap(),
        where: 'id=?', whereArgs: [venda.idVenda]);
  }
  ...
  Future<List<Venda>> getVenda() async {
    ...
    final List<Map<String, dynamic>> listadeusuario = await db.query('Venda');
    ...
    return List.generate(listadeusuario.length, (int i) {
      return Venda.fromMap(listadeusuario[i]);
    });
  }
}
